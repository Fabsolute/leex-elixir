defmodule Leex.Util.NFA do
  alias Leex.Util

  def build_combined_nfa(regex_actions) do
    {nfa, first, free} = build_nfa_list(regex_actions, [], [], 1)
    state = %Leex.NfaState{no: free, edges: Util.epsilon_trans(first)}
  ...
  defp build_nfa(regex, state, action) do
    ...
    {[%Leex.NfaState{no: end_state, accept: {:accept, action}} | nfa], next_state, state}
  end
  ...
  defp build_nfa({:kclosure, regex}, next_state, first_state, nfa) do
    ...
    {[
       %Leex.NfaState{no: first_state, edges: [{:epsilon, next_state}, {:epsilon, end_state}]},
       %Leex.NfaState{no: end_state_2, edges: [{:epsilon, next_state}, {:epsilon, end_state}]}
       | nfa
  ...
  defp build_nfa({:pclosure, regex}, next_state, first_state, nfa) do
    ...
    {[
       %Leex.NfaState{no: first_state, edges: [{:epsilon, next_state}]},
       %Leex.NfaState{no: end_state_2, edges: [{:epsilon, next_state}, {:epsilon, end_state}]}
       | nfa
  ...
  defp build_nfa({:optional, regex}, next_state, first_state, nfa) do
    ...
    {[
       %Leex.NfaState{no: first_state, edges: [{:epsilon, next_state}, {:epsilon, end_state}]},
       %Leex.NfaState{no: end_state_2, edges: [{:epsilon, end_state}]} | nfa
     ], next_state_2 + 1, end_state}
  end

  defp build_nfa({:char_class, regex}, next_state, first_state, nfa) do
    {[%Leex.NfaState{no: first_state, edges: [{Util.pack_cc(regex), next_state}]} | nfa],
     next_state + 1, next_state}
  end

  defp build_nfa({:comp_class, regex}, next_state, first_state, nfa) do
    {[%Leex.NfaState{no: first_state, edges: [{Util.comp_class(regex), next_state}]} | nfa],
     next_state + 1, next_state}
  end
  ...
  defp build_nfa(:epsilon, next_state, first_state, nfa) do
    {[%Leex.NfaState{no: first_state, edges: [{:epsilon, next_state}]} | nfa], next_state + 1,
     next_state}
  end
  ...
  defp build_nfa_alt([regex | regexes], next_state, first_state, nfa) do
    ...
    {[
       %Leex.NfaState{no: first_state, edges: [{:epsilon, next_state}, {:epsilon, next_state_2}]},
       %Leex.NfaState{no: end_state_2, edges: [{:epsilon, end_state}]},
       %Leex.NfaState{no: end_state_3, edges: [{:epsilon, end_state}]} | nfa
  ...
  defp build_nfa_lit(regex, next_state, first_state, nfa) do
    List.foldl(regex, {nfa, next_state, first_state}, fn c, {nfa, next_state, first_state} ->
      {[%Leex.NfaState{no: first_state, edges: [{[{c, c}], next_state}]} | nfa], next_state + 1,
       next_state}
    end)
  end
end
